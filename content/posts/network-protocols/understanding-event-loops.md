...
![浏览器事件循环一轮的完整流程示意图：任务执行 → 微任务检查点清空 → 渲染机会 → 下一任务](../../../public/images/event-loop-tick.svg)

*图注：一轮事件循环不是"取一个任务再取一个"，而是任务 → 清空微任务 → 可选渲染的三段式；渲染这一步可能连续跳过好几轮。*



![HTML 规范事件循环全生命周期时间线：Task -> Microtasks -> Update Rendering](../../../public/images/html-event-loop-spec-lifecycle-timeline.svg)
...
![现代浏览器调度器 (scheduler.postTask) 三级优先级队列与 INP 优化](../../../public/images/scheduler-post-task-priority-queue.svg)
