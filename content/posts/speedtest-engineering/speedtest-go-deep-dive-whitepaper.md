...
**TL;DR：** 测速结果是特定方向、计量点和时间窗口里的应用有效字节速率，不是某台服务器的带宽宣言。下行在客户端按收到字节计量，上行在客户端按已交给网络栈的字节计量；路径覆盖“终端接入网 → ISP/骨干/网间互联 → IDC 出口 → 测速主机”（边缘终止与私有云路径见 §2），BDP、TCP 反压、grace 窗口和采样统计共同决定数字。`librespeed/speedtest-go@59cff12` 用 1 MiB 随机块循环提供下行、用 `io.Copy(ioutil.Discard, r.Body)` 吸收上行，Worker 默认按 `IP_D_U` 调度 6 条下行流和 3 条上行流；它不提供全局选点、容量接纳、服务端上行权威计量和生产容量证明。

阅读约定：标题里的“从 A 到 B”是映射关系，不是调用链。本文先建立测速的观察边界（方向、计量点、时间窗、路径），再把每一项映射到 `speedtest-go` 源码里的对应位置；实现细节不改变边界定义，只决定某个边界落在代码的哪一行。
...
| jitter | 简化为 `|RTTₙ − RTTₙ₋₁|`；实际算法可用指数加权移动平均（EWMA），本标本的非对称加权见 §6.4 | 时延样本的变化，不等同于平均 RTT |
...
这些层的分母不能互换：应用层的 `B_rx/B_tx` 不包含所有协议头和重传，也不自动等于 IDC 出口或账单字节。

单位同样要写全：`1 Byte = 8 bit`；`MB` 通常按 `10^6` 字节，`MiB` 按 `2^20` 字节；`Mb/s`（兆比特每秒）与 `MB/s`（兆字节每秒）相差约 8 倍。当前 Worker 还会用 `1,000,000` 或 `1,048,576` 做显示换算，因此结果必须同时记录单位和换算因子。
...
把“测速服务”拆成十个功能，每个都有自己的定义与计量点；后文每节只放大其中一行：

| 功能 | 一句话定义 | 计量点 | 详见 |
| --- | --- | --- | --- |
| 选点（控制面） | 把一次测试绑定到确定的节点、IP 与协议 | DNS 结果、节点 ID | §2.2、§2.5 |
| 下行吞吐 | 正式窗口内客户端收到有效字节的速率 | `B_rx`，客户端 | §4 |
| 上行吞吐 | 正式窗口内客户端发出字节的速率，不是服务端确认 | `B_tx`，客户端 | §5 |
| ping | 应用探针往返耗时（本实现是 HTTP/XHR，不是 ICMP） | 客户端 | §6.4 |
| jitter | 相邻 ping 差的非对称加权 | 客户端 | §6.4 |
| 丢包与重传 | 浏览器侧不可直接观测，只能看延迟副作用；内核侧需 TCP_INFO | 服务端内核（如有） | §1.1、§7 |
| loaded latency | 满载 RTT 分布相对空闲的变化 | 客户端探针 | §2.3 |
| 遥测与结果 | 把观察值加元数据写入后端，返回可分享 ID | 服务端 DB | §9.4 |
| 多点对照 | 跨节点多次测试；单次会话内不换点 | 客户端编排 | §2.2、§10.3 |
| 并发接纳与限额 | 谁能测、测多久、多少字节（生产） | 前置层 | §10 |

读完全文后回到这张表：任何一个 Mbps 数字，都能在这十行里找到它的定义行和计量点；找不到的，就是还没被定义的口径。
...
地址族选择是另一层竞速，不在 DNS 解析里结束：双栈终端拿到 A 与 AAAA 记录后，按 Happy Eyeballs（RFC 8305，优先 IPv6、交错竞速建连，哪个先通走哪个；v3 草案再加入 SVCB/HTTPS 与 ECH 提示）决定本次连接的地址族。因此“测到 v4 还是 v6”不是配置项，而是那一次竞速的结果；两次测试可能落在不同地址族、不同路径上。§2.5 绑定清单里的“IP 版本”字段正为此存在：不记录它，跨次比较就是在比较两条可能不同的路径。
...
当前 `speedtest-go` 的 `SetServerLocation` 只获取服务器坐标等元数据，源码没有替你实现基于客户端实测的全局调度、节点健康评分或跨节点会话绑定。这是“能显示节点位置”和“能选出合适节点”之间的差别。Ookla 的选点协议是具体形状的对照：按地理取 10 个候选、对每个打 ping、选最低延迟者，多 server 测试取延迟最低的 4 个（独立实证研究，CAIDA 2024）。min-latency 竞速天然偏向 on-net 节点——而 on-net 测的是最后一段，off-net 还要经过 transit 与 IXP（同上）。所以“选点策略”本身就是被测对象的一部分：同样的客户端，min-latency 选点与固定远端是一场不同的测试。

客户端这一侧也有隐藏边界：Wi-Fi 的空口竞争、5G 的无线调度与 RRC 状态迁移、家庭网关队列、NAT/CGNAT（运营商级地址转换）和 VPN/代理都可能改变路径或可见身份。服务端看到的公网 IP 不一定对应一个设备；按公网 IP 做限流在 CGNAT 下可能把很多用户合并，按它做地域和 ISP 推断也可能失真。测速前还应说明是否存在 VPN、代理、热点共享和其他背景流量。
...
因此，第一节公式中的 `C_access` 可以理解为终端到接入网络的最后一段，严格说还应含终端自身：1 Gbps 网卡、CPU、内存与 Wi-Fi 空口都可能先于网络成为瓶颈（Livingood，APNIC 2018）；`C_path` 则覆盖接入 ISP 之后的城域/骨干、网间互联和 IDC 出口；三者不要重复计算。`C_node` 才描述测速主机和应用服务本身。要判断 IDC 是否是瓶颈，至少应把节点的 IP 版本、上游/互联、空闲与负载 RTT、丢包/重传、出口利用率和主机资源分开记录，而不是只在结果里保存一个城市名。
...
测 loaded 的方法是在上下行满载期间持续发轻量探针（例如对 `/empty` 的小 GET），取加载 RTT 分布与空闲 RTT 分布对照；差值就是这条路径被测速流自己撑起来的队列。若 loaded P50 比 idle 高出数倍而吞吐仍“达标”，结论应写成“带宽够、延迟结构差”，而不是“网络好”——这正是 bufferbloat 诊断的标准动作：用两个分布的差值代替单一 RTT 数字。

业界已经把这组概念固定成命名，值得直接借用（Ookla 方法页，2025-10）：Minimum（多次 ping 取最低，最乐观）、Unloaded（空载经典值）、Primary（空闲探针的 IQM 中心趋势）、Multi-Server（多 server 平均，含对等互联信号）、Loaded（满载 RTT，分上下行）、Primary Loaded（传输期间的 IQM）。对照之下，本标本 §6.4 的 ping 是 Minimum 口径，jitter 是自有加权；它没有 Multi-Server 与 Loaded 口径。跨工具比较延迟前，先把六个名字对齐，否则一个报 Minimum、一个报 Loaded，数字差出数倍也在意料之中。
...
RTT 越大，单流填满 BDP 越难：量纲关系与窗口数学见 §3.1，这里不再用假设数字重复演示。
真实比较必须把同一终端、同一时间段、同一协议下的实测 RTT、吞吐、丢包/重传和失败率填回上表。

BGP 路由选择以网络策略和工程配置为主，AS 路径短不等于 RTT 低、队列短或可用容量大；要比较两个节点，至少应固定测试端、记录解析地址和 IP 版本，并用重复的空闲/加载 RTT、吞吐、丢包/重传和失败率做对照；“同城同运营商”只能作为一种实验组合，不能作为天然的质量保证。

### 2.5 先记住一次完整测试的总览

一次默认测试按 `IP_D_U` 走完三段：`getIP` 只取身份与位置元数据，不测速；下行用 6 条流拉 `/garbage`，客户端按收到字节和正式窗口计算 `R_down`；上行用 3 条流推 `/empty`，浏览器按发出字节计算 `R_up`，服务端只读取丢弃；遥测默认关闭，打开后才提交观察值。流数的实际并发度还受浏览器同源连接池与协商出的 HTTP 版本约束（见 §9.3），6 流不等于 6 并发。

后面的 §4、§5、§9 分别放大这三段的计量点、反压和状态机。
...
BBR 是另一类选择：它不以丢包为主要信号，而是显式估计瓶颈带宽与最小 RTT，按模型设置 pacing 速率和在途上限，经历 STARTUP、DRAIN、PROBE_BW、PROBE_RTT 状态。对测速的直接含义是：服务端操作系统的拥塞控制选择会改变客户端看到的爬坡形状，grace 窗口想跳过的“启动段”在 CUBIC 与 BBR 下不是同一条曲线；比较两条路径前，先确认两端各跑什么算法。M-Lab NDT7 甚至把这一点写进方法学：在服务端启用 BBR，并用 TCP_INFO 取内核观测，与客户端应用记账对照。独立对照实验补了一个定量注脚：旧 NDT 用 Reno 时单流难以打满，换 BBR 后单流在 2 Gbps 内可达链路 95% 饱和（MacMillan 等，arXiv 2205.12376）——拥塞算法威胁的是“能不能打满”，BDP 威胁的是“单流要多大窗口”，两者正交。
...
`getRandomData` 使用 `crypto/rand` 填充这 1 MiB，`garbage` Handler 在请求路径里重复写它。服务端默认写 4 个 chunk；`ckSize` 可以表达客户端想要的 chunk 数，但大于 1024 时被服务端钳制。因此本地取证得到：默认 4,194,304 字节，`ckSize=1` 得到 1,048,576 字节，`ckSize=99999` 得到 1,073,741,824 字节。边缘行为直接读代码即得：`ckSize=0` 或负数时循环零次、返回空响应 200；非法字符串解析失败则回退默认 4。
...
反压是对称的，上面只讲了读侧。下行方向同样会堵，只是堵在服务端的写侧：

| 方向 | 服务端队列 | 满了之后 | 客户端看到什么 |
| --- | --- | --- | --- |
| 下行 | SND-Q（socket 发送队列） | `w.Write` 阻塞，持有请求的 goroutine 停住 | 进度停顿，但连接不断 |
| 上行 | Recv-Q（socket 接收队列） | 通告窗口缩小乃至 zero window | upload 进度停顿 |

“客户端等字节、服务端堆 goroutine”是下行慢的典型双向症状：瓶颈不在 `C_path`，而在服务端发送跟不上时，队列只是从 Recv-Q 换到了 SND-Q。因此 §4 的“服务端提供字节”与 §5 的“服务端吸收字节”不是两种运气，而是同一套流控在两个方向的投影；诊断慢速时，先问堵的是 `C_path` 还是两端的 SND-Q/Recv-Q，再看 cwnd。
...
浏览器里，`performance.now()` 提供相对页面生命周期的单调时间；`Date.now()` 是墙上时间。但“高分辨率”是有部署条件的：Spectre 之后浏览器把它 coarsen 了，非跨域隔离上下文 100 µs，隔离上下文 5 µs（Firefox/Safari 更粗）。对 200 ms 的状态更新节拍而言，100 µs 粒度引入的误差可忽略，不改变吞吐结论；但它不能被写成无条件纳秒级。需要特别区分设计建议和标本事实：当前 `speedtest_worker.js` 的下行、上行和大部分日志时间使用 `new Date().getTime()`，ping 会尝试从 Resource Timing 取 `responseStart - requestStart`，失败时再回退到 Date 差值。因此不能把“测速服务应该使用单调时间”改写成“这个 Worker 已经端到端使用单调时钟”。
...
这改变的是测试持续时间，不是字节计量点。它也意味着两个配置不同的客户端，可能在不同的窗口长度上得到不同的结果。时长本身不是中性参数：Ookla 指出 5G 载波聚合需要持续需求才能触发，短测试量不到最大载波配置——测多久是方法的一部分。
...
固定参数与自适应是两种设计：本标本流数、grace、窗口全部固定，只用 time_auto 缩短快连接；独立对照实验（MacMillan 等，arXiv 2205.12376）显示 Ookla 反其道而行——按实测吞吐动态增减连接数（高 RTT 下用到 8 条）并延长测试时长（中位数 3.5 秒拉到 15.7 秒），其旧版 HTTP 客户端则采样 20 次、去最高 2 个与最低 25%。没有哪种天然正确：固定参数可复现，自适应保饱和；但比较两个工具的数字前，必须先问双方各用哪种——否则是在比较采样器，不是在比较网络。
...
| M-Lab NDT7（对照设计） | 浏览器可达（WebSocket+TLS） | 单 TCP 连接上的应用 goodput，服务端另取 TCP_INFO 内核观测 | 强制单流、服务端 BBR（可用时）、单次 10 秒上限、可提前结束 | 非本标本；与本实现形成单流/多流对照 |

M-Lab NDT7 是有意站在另一端的设计：单连接、服务端 BBR、内核与应用双记账。它证明同样的“带宽问题”换一套协议与计量点会变成另一场测试，这正是“协议决定被测对象”的最直接例证。
...
公有云不是 IDC 的反义词：云厂商的虚拟机和容器仍运行在数据中心，只是把物理主机、交换路径和部分出口策略抽象成云资源。公有云里的 Region 通常是地理和服务编排边界，Availability Zone（可用区）是区域内的故障隔离/部署边界；它们帮助描述资源位置和冗余，却不能单独告诉读者客户端会从哪个运营商出口进入。云厂商文档也会把实例类型、多流、网关和网络带宽作为不同条件，因此“实例标称 10 Gbit/s”仍不是某个客户端能够获得的固定吞吐。AWS 文档是一个具体例子：经 Internet 网关的流量单流上限 5 Gbit/s（多流另算），小规格实例还有 baseline/burst credit 机制——单客户端测速首先撞到的是这类流级上限，而不是网卡数字。公有云的 Region/AZ、实例网络额度和出口计费要按具体厂商与产品核对，本文不把任何厂商的默认值推广成通用事实。
...
成本也要沿同一条字节路径核算：应用层 goodput 只是其中一个观测量，不能拿测速结果直接乘一个“带宽单价”当作账单分母（分母不可互换见 §1）。
...
Main thread 不是孤立的业务参与者。它是浏览器 UI/控制线程：创建 Worker，发送 `start + settings`，每 200 ms 发送一次 `status`，接收 Worker 返回的 JSON 后调用 `onupdate` 更新界面；当 `testState >= 4` 时停止轮询并调用 `onend`，用户取消则发送 `abort`。这条 200 ms 轮询是主线程的 `setInterval`（`speedtest.js`），它假设页面在前台：Chrome 对隐藏超 5 分钟页面的链式 timer 做 1 分钟级 intensive throttling，后台久置的界面轮询会被打乱——该限制针对 Window 计时器，Worker 内的数据流不走这条路径，但后台测的状态推进不可与前台结果直接比较。真正的大块 GET/POST 数据流在 Worker 和 Go Handler 之间，Main thread 只搬运控制消息和状态快照，所以图中它的位置表达的是“控制面”，不是“数据面”。
...
流数还要和浏览器的同源连接预算一起读：Chromium 对普通连接池默认每主机 6 连接（源码 `g_max_sockets_per_group`，HTTP/2 多路复用不受此限）。含义是：在 HTTP/1.1 下，6 条下行 XHR 会占满同源连接预算，上行与 ping 只能排队或复用；切到 HTTP/2 后多流共享一条连接，“流数”从连接数变成共享连接上的并发请求数。所以 `xhr_dlMultistream=6` 的实际并发度由协商出的 HTTP 版本决定——这是 §7“协议决定被测对象”在 Worker 参数上的投影。
...
## 十一、发展脉络：每一代都重新定义一次被测对象

每一代测速都重新定义了一次“被测对象”，时间线如下：

![测速演进时间线：每一代都重新定义被测对象，标本位于开源自托管一支](../../../public/images/speedtest-whitepaper-evolution.svg)

时间线只标各代改变的被测对象，不评价份额与优劣；年份依据参考资料中的一手来源。

三条演进线值得记住：插件 → 浏览器原生（Flash → HTML5），能力约束从插件回到浏览器手里；闭源 → 开源自托管（Ookla → LibreSpeed/go），选点与容量责任从厂商回到部署者；单口径 → 双记账（ndt7 的应用 + 内核），“谁观测”本身成为设计变量。本文的标本（speedtest-go）站在第二条线的末端：它回答“搭好节点后一次测试如何计量”，不回答“去哪里测、能承载多少”——那是第一、三条线留给部署与平台层的问题；其中 fast.com 证明“功能做减法”本身也是一种方法学声明。
...
6. **证据与重复性**：源码、实验或生产观测分别支持什么，是否有重复运行、失败率和异常原因；聚合层是否防偏：每用户先平均、排除受控环境测试、按设备能力过滤，否则众包数字只是“愿意测速者”的画像（聚合方法见 Ookla 方法页）。
...
1. **grace 窗口改变分布多少？** 假设：延长下行 grace 会系统性抬高短测试均值。固定节点、流数、时间段，只改 `time_dlGraceTime`（如 0 s / 1.5 s / 3 s），观测全窗口均值与 P50 的漂移；done：同一条件下重复 10 次，漂移方向一致且失败率不变。结论不外推到上行或跨节点比较。
2. **浏览器 `B_tx` 与服务端读取值差多少？** 假设：代理缓冲或慢消费会让两者分叉。在 `/empty` 侧记下 `io.Copy` 返回的字节数，与客户端 `upload.onprogress` 累计值对账；done：直连、反向代理、慢消费三种条件下各重复 10 次，给出差值分布。结论不外推到公网路径。
3. **多流在什么时候停止帮忙？** 假设：超过路径共享瓶颈后加流只增加抖动。在受控链路（如上限 1 Gbit/s）上从 1 流加到 12 流，观测总 goodput、P90 RTT 与重传；done：找到 goodput 拐点并重复验证。结论不外推到无线接入或公网。
...
- [RFC 9438：CUBIC（Linux/Windows/Apple 默认拥塞控制，更新 RFC 5681）](https://www.rfc-editor.org/rfc/rfc9438)
- [IETF CCWG：BBR Congestion Control 草案（BBRv3）](https://datatracker.ietf.org/doc/draft-ietf-ccwg-bbr/)
- [RFC 7323：TCP Extensions for High Performance](https://www.rfc-editor.org/rfc/rfc7323)
- [RFC 4271：A Border Gateway Protocol 4 (BGP-4)](https://www.rfc-editor.org/rfc/rfc4271)
- [RFC 9111：HTTP Caching](https://www.rfc-editor.org/rfc/rfc9111)
- [RFC 7540：HTTP/2（已被 RFC 9113 废止，保留作历史对照）](https://www.rfc-editor.org/rfc/rfc7540)
- [RFC 9113：HTTP/2（现行）](https://www.rfc-editor.org/rfc/rfc9113)
- [RFC 9000：QUIC](https://www.rfc-editor.org/rfc/rfc9000)
- [RFC 9114：HTTP/3](https://www.rfc-editor.org/rfc/rfc9114)
- [RFC 8446：The Transport Layer Security (TLS) Protocol Version 1.3](https://www.rfc-editor.org/rfc/rfc8446)
- [MDN：High precision timing（performance.now 的 coarsen 规则）](https://developer.mozilla.org/en-US/docs/Web/API/Performance_API/High_precision_timing)
- [Chrome：后台页面计时器节流（intensive throttling，2021）](https://developer.chrome.com/blog/timer-throttling-in-chrome-88)
- [M-Lab：ndt7 Protocol（单流 WebSocket+TLS、BBR、TCP_INFO 双记账的对照设计）](https://github.com/m-lab/ndt-server/blob/main/spec/ndt7-protocol.md)
- [RFC 8305：Happy Eyeballs v2（双栈竞速建连，优先 IPv6）](https://www.rfc-editor.org/rfc/rfc8305)
- [LibreSpeed：开源自托管测速（2016 起）](https://github.com/librespeed/speedtest)
- [Ookla：About Speedtest](https://www.speedtest.net/about)
- [Ookla：Speedtest Methodology（延迟分类、动态扩展、聚合方法，2025-10 更新）](https://www.ookla.com/resources/guides/speedtest-methodology)
- [MacMillan 等：Ookla 与 NDT7 对照实验（arXiv 2205.12376）](https://arxiv.org/abs/2205.12376)
- [CAIDA：Ookla 测速基础设施实证（2024，含选点与 on-net 分析）](https://www.caida.org/catalog/papers/2024_empirical_characterization_ooklas_speed_test/)
- [APNIC（Livingood）：千兆时代的测量挑战（2018）](https://blog.apnic.net/2018/06/21/measurement-challenges-in-the-gigabit-era/)
- [Netflix TechBlog：Building fast.com](http://techblog.netflix.com/2016/08/building-fastcom.html)
- [M-Lab：Introducing ndt7](https://www.measurementlab.net/blog/ndt7-introduction/)
- [Chromium：同源普通连接池默认上限（源码）](https://github.com/chromium/chromium/blob/main/net/socket/client_socket_pool_manager.cc)
