...
**TL;DR：** webhook 接收端三断言：合法投递执行恰好 1 次（`executions=1`），同 id 复投回 `200-duplicate` 不重执，篡改体与错密钥双双 `401`。4 断言全过。两条铁律：比对必须 `timingSafeEqual`（防时序侧信道），执行必须先查重（至少一次投递是常态，重试、超时、LB 都会复投）。
...
`experiments/webhook-hmac/hook.mjs`，`evidence/webhook-hmac/2026-09-14-local/run.out`，4 PASS。
