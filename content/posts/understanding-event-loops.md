...
我们常用"一条不断取任务的循环"解释事件循环。这个比喻适合入门，却掩盖了真正重要的细节：浏览器同时协调**任务队列**、**微任务检查点**、**渲染机会** 和多个**执行环境**（同源窗口、iframe、web worker 各自有事件循环，但共享进程级调度）。
...
浏览器里"事件循环"不是一个。每个同源窗口、iframe 和 worker 都有独立的事件循环，但它们共享进程级的任务调度与渲染时机。`postMessage` 在窗口间投递的消息，会进入目标窗口的任务队列，按目标环境的轮次执行——这就是为什么 `postMessage` 常用于"把工作踢到下一个宏任务"（比如避免在事件回调里做重活、或者跨 iframe 传数据时保持顺序）。

理解这一层的价值在排查跨窗口联动卡顿时：父窗口的长任务会推迟子 iframe 的渲染机会，因为渲染时机由共享的帧时钟决定（同源窗口是否共享事件循环因浏览器实现而异，此行为在 Chrome 中较明显）。单独测量子页面指标都正常、合在一起却掉帧，问题往往在这里。
...
// 输出总是 m1 → m2 → m3：postMessage 进入目标窗口的任务队列，
// 每轮事件循环派发一条；若队列里积压着长任务，后面的消息只能排队等。
```
