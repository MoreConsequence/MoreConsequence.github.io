...
- 金丝雀在旧版本仍健康、路由规则可原子切换且控制面反应足够快时，才可能快速摘掉 canary；这不是任何平台都保证的秒级合同。
- 蓝绿可以切回旧环境，但数据库 schema、消息格式和缓存格式必须向前/向后兼容；代码流量切回不等于数据已经回滚。
...
灰度（ramped）按用户/地域分批放大。分桶通常要让同一用户在观察窗口内稳定落在同一批次，否则两次请求分处新旧版，用户行为可能不一致。这里的“稳定”不一定等于把连接永久粘在某个实例：如果 session、缓存和数据库 schema 采用共享且向后兼容的合同，蓝绿或滚动也可以安全切换；如果状态只在进程内，任何发布策略都会放大丢失风险。工程难点不在“比例”，而在身份哈希、状态迁移和新旧版本同时读写同一数据的兼容性。
