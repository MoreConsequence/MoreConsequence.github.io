...
![端到端时延归因实测：服务端处理、网络往返 (RTT) 与客户端事件循环调度](../../../public/images/latency-attribution-three-stages-server-network-client.svg)
...
![端到端时延归因全景分解：客户端建立 -> 网关排队 -> RPC 调用 -> 数据库事务](../../../public/images/latency-breakdown-network-queue-exec-flow.svg)
...
![通用可伸缩性定律 (USL)：并发排队延迟 (Contention) 与缓存一致性 (Coherence) 拐点](../../../public/images/universal-scalability-law-contention-coherence.svg)
...
把这套测量搬到你自己的服务里，只需要一个下午：挑最慢的接口，按基线 / 关 keepalive / 注入已知 think / 放大 body 四轮各打 300 个串行请求，两张分布表一对比，该优化哪一段、不该碰哪一段就都有了答案。归因靠差值，不靠直觉——直觉在延迟问题上几乎总是站在最吵的那一段一边。
