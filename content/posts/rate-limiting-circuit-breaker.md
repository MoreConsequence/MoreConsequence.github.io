...
选择建议：**入口限流用令牌桶**（允许自然的突发，如秒杀瞬间的集中请求）；**出站保护用漏桶或带缓冲的令牌桶**（数据库连接、消息队列生产者最怕毛刺）；固定窗口只适合"成本敏感、精度无所谓"的粗粒度场景；单机场景 Guava 的 `RateLimiter`、分布式场景 Redis + Lua 是标准答案，后者把"计数 + 判断"放进原子脚本，避免竞态。

### 2.2 阈值不是拍脑袋，是算出来的

限流阈值 = **容量基线 - 安全余量**。正确流程：

1. 压测：用比生产大一倍的流量压本服务，找出"延迟开始拐弯"的 QPS，记作 `P`（比如 2000 QPS 时 P99 从 20ms 跳到 200ms，2000 就是拐点）。
2. 留余量：生产阈值 = `P × 0.7` 左右（具体看拐点陡峭程度，拐得越陡余量越大）。假设 2000 → 生产限 1400 QPS。
3. 按维度切分：全局限流 1400，加上"单用户 10 QPS、单 IP 50 QPS"的细化限流——全局限流挡不住单一来源的刷量，维度限流挡得住。
4. 压测回归：每次容量变化（发布、加节点、改配置）后重新压，阈值跟着基线走。

"限流 50% 流量"这种拍脑袋数字的事故，本质是跳过了第 1、2 步。没有基线就没有阈值。
