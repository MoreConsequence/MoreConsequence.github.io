...
naive 每轮超卖数不固定（67~143）不是误差，是它该有的样子——超卖量由调度交错决定，数值每次不同，但"必然超卖"是结构性的；cas 与 luascript 三轮全部为 0。注意 `ops/s` 是进程内模拟吞吐，不代表真实 MySQL/Redis 延迟：本机一次结果只证明正确性差异，不构成性能分界线。真实依赖的延迟对比（DB 条件 UPDATE 的 P99、Redis EVAL 的 P99、MQ 端到端延迟）不在本文范围——正确性语义是本文的账，性能排序需要真实存储与监控才能写。
