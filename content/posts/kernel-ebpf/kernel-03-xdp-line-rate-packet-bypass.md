...
|                         XDP 三大运行模式对比                            |
|                                                                         |
|  1. Offloaded 模式 (SmartNIC 硬件卸载): 直接在智能网卡 NPU 上执行 (0 CPU)|
|  ────────────────────────────────────────────────────────────────────── |
|  2. Native / Driver 模式 (驱动原生模式): 在网卡驱动初始化 DMA 处执行 (最优) |
|  ────────────────────────────────────────────────────────────────────── |
|  3. Generic 模式 (通用回退模式): 在 SKB 分配后执行 (用于测试与老旧驱动)    |
+-------------------------------------------------------------------------+
```
...
![AF_XDP 零拷贝内存架构](../../../public/images/af-xdp-zero-copy-umem.svg)
