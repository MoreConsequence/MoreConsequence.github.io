---
title: "主从复制延迟的账单：读路径设计的三种姿势"
description: "复制延迟不是单一的秒数，而是 binlog 拉取、relay、重放和故障恢复共同形成的预算；用读主、版本校验、半同步三种姿势承接不同的一致性要求。"
...
**TL;DR：** 主从复制延迟是异步复制的**固有账单**，不是一个可以照抄的固定秒数：主库提交后，从库还要经过网络拉取、relay log 写入、SQL 线程重放和可能的恢复重试。读路径可以分成三种姿势：**读主**（放弃读扩展换强一致）、**读从 + 版本校验**（容忍延迟，校验失败重读主）、**半同步 + 读从**（用写延迟买读一致性）。选哪一种，取决于数据的一致性预算：昵称可以滞后，余额和库存的容忍窗口可能接近零。预算写进代码和监控，事故复盘才有对照。
...
反例看余额扣减：假设从库落后几百毫秒，用户看到的"可用余额"仍可能是旧值。两个并发请求都读到"够扣"，扣减逻辑基于同一份旧余额执行，超扣的后果直接出现在用户账单上，而不是监控面板上。校验类请求更隐蔽：读从库判"余额是否充足"，通过后在主库扣减才发现透支，只能回滚或欠账。这类错误的主语是用户，不是某个固定延迟数字。
...
type Product struct {
    ...
    Version uint64 // 单调递增的行版本号；不要把任意时间戳直接当版本
}

func (s *Service) GetProduct(ctx context.Context, id int64, requiredVersion uint64) (*Product, error) {
    // 1. 默认读从库
    ...
    // 2. 版本校验：调用方从写响应/会话 token 带来的版本未满足，就读主库
...
版本校验的本质是**把"延迟"从黑盒变成可判定的量**：不关心复制到底慢了多少，只关心"我读到的版本是否满足调用方带来的最低版本"。低于 `requiredVersion` 的读自动升级为主库读。代价是逻辑多一层，且这个版本 token 必须来自可靠的写响应、会话或主库状态，不能由任意实例本地猜测。
...
选姿势之前先回答一个问题：**这行数据滞后一个可观测窗口，业务后果是什么？** 把数据按后果分类，预算就出来了：
