...
某内部网关的优化过程，完整走一遍这套流程：

```bash
# 采样 30s，发现 CPU 最高的是用户态 memcpy（见上文火焰图）
perf record -F 99 -g -p 21457 -- sleep 30
perf script | ./FlameGraph/stackcollapse-perf.pl | ./FlameGraph/flamegraph.pl > flame.svg
# 判读：handler → read_socket(55%) → sys_read(48%) → copy_to_user(45%) → memcpy(44%)
```

归因结论：网关每次把 64KB 读缓冲从内核拷到用户态再原样发出——数据根本没有被业务处理过，一次完整的"无效拷贝"。修法不是优化 memcpy，而是**把 read+write 换成 splice**（内核态页缓存 → socket 直连，参考[零拷贝路线图](/writing/zero-copy-sendfile-io-uring)的对照表）。改造后重新采样：`memcpy` 从 44% 降到 9%，网关吞吐提升 2.3 倍。**验证环节与采样环节同样重要**——没有第二次采样，就无法证明优化真的落到了 CPU 时间上。
...
跑完 diff，顺手把结论固化成回归清单：本次优化的目标函数、优化前占比、优化后占比、验收阈值（比如"memcpy 占比必须从 44% 降到 20% 以下"）。下次合入新的改动时重跑同一套采样脚本，用 diff 检查有没有让老热点回潮。**优化是一次性的，回归检查是长期的**——这也是第七节流程图最后一步"写进回归清单"的意思。
...
**"CPU 占用低 = 没有性能问题"是错的。** on-CPU 火焰图只能证明"CPU 没被烧"，证明不了"没有瓶颈"。举一个我踩过的例子：一个消息处理服务，CPU 稳定在 15%，客户端却频繁超时。火焰图几乎全空，直觉上"资源这么空闲怎么会慢"。跑 offcputime 才发现 90% 的阻塞时长堆在一个 `fsync` 调用栈上——每条消息落库都强制刷盘，磁盘是瓶颈，CPU 只是陪跑。**on-CPU 回答"谁烧了 CPU"，off-CPU 回答"谁在等、等什么"，两个视角合起来才覆盖线程的全部时间。**
