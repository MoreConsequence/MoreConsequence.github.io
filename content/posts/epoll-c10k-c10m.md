...
最直接的是用现成工具压一万连接，同时看系统态占比：

```bash
wrk -t4 -c10000 -d10s http://localhost:8080/
# 观察高连接数下 CPU 的 sys 占用：epoll 模型下 sys 占比低而稳定
```

或者用 `perf top` 对比两种模型的热点：epoll 路径热点集中在 `epoll_wait` 相关的内核函数，而 select 路径下 `do_select` 明显占比拉高——你就能'看见'扫描全量 vs 取就绪的差别。

**诚实注明**：如果压测连接都是空闲的，epoll 与 select 的差距会很小——差别只在"连接数大 + 同时就绪比例低"时显形。别拿空闲连接下的数字当结论。
...
下一步：别信宣传，自己 `wrk -c10000` 压一次，同时跑 `perf top` 看模型——你手上那台几核机器，几分钟就能告诉你 epoll 的账本长什么样，顺手观察 `epoll_wait` 返回次数是如何跟随就绪数而不是连接数的。
