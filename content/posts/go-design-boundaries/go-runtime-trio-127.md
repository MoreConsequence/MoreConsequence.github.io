...
**TL;DR：** Go 1.27 的运行时改动有一个共同方向——把隐式行为显式化：timer 通道恒为同步无缓冲（`cap==0`、恰好投递一次，`Stop`/`Reset` 语义锁定）；panic 的 traceback 头自带 pprof 标签（`{tenant: acme}`，`tracebacklabels=0` 可关）；`slog` 按级别过滤、按 `With` 透传字段。三组断言全过（3+2+3），环境均为本机 go 工具链。
...
`NewJSONHandler(w, {Level: Info})` 下 `Debug` 被过滤、`Info` 输出合法 JSON（含 `level` 与自定义字段），`.With` 透传每条记录（`experiments/go-slog-filter/slog_test.go`），3 PASS。级别来自部署配置，字段字典文档化，两者别混在一起 hardcode。
