...
import { ERROR_CODES, toApiError, type Order } from "./orders.ts";
import { BoundedInMemoryStore, type OrderStore } from "./store.ts";
import { Metrics, type MetricOutcome } from "./metrics.ts";

const operationFor = (method: string, path: string) => {
  if (method === "GET" && path.startsWith("/orders/")) return "orders_get";
  if (method === "POST" && path === "/orders") return "orders_create";
  return `${method.toLowerCase()}_${path.replaceAll("/", "_").replaceAll(/[^a-zA-Z0-9_]/g, "") || "root"}`;
};

const outcomeFor = (status: number): MetricOutcome => {
...
export const createApp = (store: OrderStore, metrics = new Metrics()) => {
  const app = new Hono();
  ...
  const zodHook: Hook<z.infer<typeof createOrderBody>, Env, string, "json"> = async (result, c) => {
    if (result.success) return undefined;
    metrics.inc("orders_create_validation_failed");
    c.status(400);
    return c.json(toApiError("INVALID_BODY", "request body failed validation",
      result.error.issues.map((i) => ({ path: i.path.map(String), code: i.code, message: i.message }))
    ));
  };
  ...
  app.post("/orders", zValidator("json", createOrderBody, zodHook), async (c) => {
    ...
    const result = await store.saveByKey(
      v.idempotencyKey,
      order,
      JSON.stringify({ sku: v.sku, customerId: v.customerId, qty: v.qty }),
    );
    if (result.conflict) {
