...
`kv_cache_budget.py` 用纯标准库把「每 token 的 KV 字节 → 不同 seq_len × batch 的显存 → 并发上限」算成一张表。核心公式只有一步，可手算复核：
...
可选项：`--vram`（默认 40）、`--dtype`（权重 dtype，fp16/bf16/fp8/int8，默认 fp16）、`--kv-dtype`（KV cache dtype，缺省同 `--dtype`）、`--seq`（默认 4096,8192,32768）。

`--kv-dtype` 与 `--dtype` 分开，是为了把「只量化 KV」和「权重量化」分开算：只量化 KV 时（如 `--dtype fp16 --kv-dtype fp8`）权重保持 16GB 不动，KV 字节减半；`--dtype fp8` 则是权重与 KV 一起按 1 字节计。真实 serving 里两者是独立的（KV 量化不动权重，权重量化可以搭配 KV fp16），按需要选。
...
权重(估算): 16.0 GB  固定开销: 4 GB  可用给 KV: 20.0 GB
 ...
 seq_len       单请求 KV   batch=1 总 KV       并发上限      并发时 KV 上限
------------------------------------------------------------------
    4096     0.250 GiB        0.250 GiB         80      20.000 GiB
    8192     0.500 GiB        0.500 GiB         40      20.000 GiB
   32768     2.000 GiB        2.000 GiB         10      20.000 GiB
```

KV 字节从 128 KiB/token 减到 64 KiB/token，并发上限正好翻倍（40→80、20→40、5→10）。代价是精度，见文章第五节的精度讨论与【本机实测待补】。
...
- 权重按 `参数 × 字节/元素` 估算，不含 embedding 与输出头的精度细节。
