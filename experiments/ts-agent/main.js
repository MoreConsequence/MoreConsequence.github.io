...
async function executeBatch(calls, timeoutMs) {
    const withTimeout = calls.map((call) => Promise.race([
        runTool(call),
        sleep(timeoutMs).then(() => ({ ok: false, error: "timeout" })),
    ]));
...
async function agentLoop(maxRounds, timeoutMs) {
