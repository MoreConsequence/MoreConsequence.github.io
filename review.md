...
## 四十二、2026-09-07 pastel 批次整体规范化：201 张迁入 skill 设计体系

用户抽查指出：大量文章配图仍是"3~4 个彩色方块 + 底部大片留白"，配色不符合 skill。核实属实——第四十一节前的迁移对这批（bd8786c 生成器批次，201 张）只映射了中性色，保留了粉彩分类色与卡片栅格。本节登记彻底规范化：

| 处理 | 规模 | 说明 |
| --- | --- | --- |
| 粉彩调色板 → 规范角色 | 201 张全部 | 彩色面板/卡片 → 白底 + 规范描边（橙红系→珊瑚 accent 焦点，蓝绿紫系→ink/muted/soft）；彩色标题文字→ink、正文→muted、浅色→soft；深色终端块保留，块内彩色文字统一浅色（修复 on-dark 判定对省略 x/y 属性矩形失效的问题，1,796 条文字归位） |
| 去 shadow + 去 dot-grid | 201 张 | skill 反模式：阴影出局；博客内页不用点阵背景 |
| 断链卡片文字归位（第三类签名） | 75 张 / 656 处 | 与前批 46 张同源的"+卡片 y 脱钩"bug，但该子类文字省略 x 属性或卡片宽度非 255，此前两轮扫描均未命中；通用签名（白卡 + 后继兄弟文字 y<100）全库扫描修复，复扫 0 残留 |
| 空面板移除 + 面板按内容收缩 + 画布裁高 | 空面板 1 张、面板收缩 1,546 处、画布裁高 203 张 | 消除"卡片底部大片留白"；收缩后同列下方元素整体上移 |
...
| `node-worker-offload` | 从 Go 到 TypeScript | busy 推迟 40ms，下放回 1ms（P0-05 闭环） | `experiments/node-worker-offload/demo.mjs` 2 断言 | 池调度、transfer 性能 |
| `go-fuzz-parser-crash` | Go 的设计边界 | 0.4s 抓空 key，30s/970 万次确认 | `experiments/go-fuzz-parser/` 语料入库回归 | 长时间 fuzz、非纯函数 |
| `http-range-resume` | 网络协议 | 206 + 精确 Content-Range，524288 处续接 | `experiments/http-range/` 2 测试 | 多段、CDN 交互 |
| `cors-preflight-trio` | 网络协议 | 204 三件套，凭据回显具体源 | `experiments/cors-preflight/cors.mjs` 4 断言 | 浏览器差异、代理改写 |
| `read-your-write-session` | 系统设计手记 | 粘性 99.41%、版本门回主 0.59% | `experiments/read-your-write/ryw.py` 合成延迟 4 断言 | 真实延迟、主库容量 |
...
| `http-chunked-flush` | 网络协议 | 首字节 1.1ms vs 302ms | `experiments/http-chunked/` 2 测试 | 公网、代理缓冲 |
| `crdt-gcounter-merge` | 系统设计手记 | 顺序无关 23、幂等、槽位隔离 | `experiments/crdt-gcounter/gcounter.py` 3 断言 | 同步延迟、槽位爆炸 |
| `llm-15-context-eviction` | 大模型后端 | 恒 8/20，策略只改分布（合成） | `experiments/context-eviction/evict.py` 4 断言 | 真实模型召回 |
| `service-webhook-hmac` | 系统设计手记 | 签名+去重，同 id 不重执 | `experiments/webhook-hmac/hook.mjs` 4 断言 | 密钥轮换、重放窗口 |
| `retry-budget-math` | 系统设计手记 | 1.02x/1.248x/熔断回 1.0 | `experiments/retry-budget/retry.py` 纯算术 4 断言 | 退避抖动、半开恢复 |
...
## 四十六、2026-09-14 全库打分与修法（KEEP 122 / POLISH 115 / DEEPEN 14 / DEL? 26）

规则（`evidence/content-scores/2026-09-14-local/scores.csv`，277 篇全量，可复算）：正文 0–5 分（500/1000/1500/2500/4000 字档）+ 代码块 0–2 + 证据链接 0–1 + 参考资料≥3条 0–1 + 图/表 0–1 = 10 分制。≤3 删候选、4–5 加固、6–7 补引用补图、≥8 保持。

校准结论（与“标题长+数字怪+内容短”的体感交叉验证）：

1. **删队列实际为空。** 26 个低分全是有独特问题 + 可运行证据的微篇，按“先优化” ladder 不够删除线。唯一零被引零证据的是索引篇（设计如此）。本次零删除。
...
3. **补引用队列修正（2026-09-16）**：§46 初版称“41 篇无参考资料节”，系 `^## 参考资料$` 精确标题正则误报（漏掉“六、参考资料：…”等变体标题）。重扫确认：**全库 production 篇篇有参考资料节**，剩余工作是把 <3 条的补到 3 条以上（dsh/oract 系只能引项目内文档，逐篇核实，不批量编）。rate-limiting 已补 RFC 6585 §4（rfc-editor 全文核实）。
