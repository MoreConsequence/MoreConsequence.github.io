...
| 主题 | 核心问题 | 验证方式 |
| --- | --- | --- |
| 超时与取消的真语义 | AbortSignal 连的是"请求"还是"整条 Agent 循环"？取消后工具副作用怎么办 | 可运行示例 + 源码阅读（undici/AbortSignal） |
| 写工具的幂等键 | 上次只承诺了"先写幂等键"——现在落地：200/201 语义、重放检测、并发窗口 | 接 service-idempotency 实验 + 单测 |
| 对话级预算 | 从单轮超时升级为整会话 token/步数/费用预算，超预算的优雅降级 | 可运行原型 |
...
| 时钟与排序 | Lamport/向量时钟直觉、逻辑时钟 vs 墙钟、因果序 | 可运行模拟 |
| 共识直觉 | Paxos/Raft 到底解决了什么、为什么多数派、脑裂 | 模拟器（raft.js 类）复现 |
| 分布式事务三选一 | 2PC / SAGA / Outbox 的故障模型与语义承诺 | 对照实验 + 故障注入 |
...
| eval 集设计陷阱 | 数据泄漏、标签噪音、样本量幻觉 | 对小数据集做统计演示 |
